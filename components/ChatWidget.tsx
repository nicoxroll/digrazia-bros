            ...
            {messages.map((m, i) => (
              <div key={i} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'}`}>
                ...
                <div className={`max-w-[90%] p-4 rounded-2xl text-sm leading-relaxed prose prose-sm prose-stone ${
                  ...
                  {m.image && (
                    <div className="mt-4 relative group cursor-pointer rounded-xl overflow-hidden" onClick={() => setFullscreenImage(m.image)}>
                      <img src={m.image} alt="Generated Design" className="w-full h-auto rounded-xl shadow-sm hover:scale-[1.02] transition-transform" />
