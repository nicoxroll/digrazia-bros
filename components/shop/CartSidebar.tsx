...
export const CartSidebar: React.FC<CartSidebarProps> = ({ items, isOpen, onClose, onUpdate }) => {
  ...
  return (
    <>
      <div className={`fixed inset-0 bg-black/40 backdrop-blur-md z-50 transition-opacity duration-500 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={onClose} />
      <div className={`fixed right-0 top-0 h-full w-full max-w-lg bg-white z-[60] shadow-2xl transition-transform duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-10">
            {items.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
                ...
                <p className="text-nude-300 font-serif italic text-2xl">The basket is empty.</p>
              </div>
            ) : items.map((item) => (
              <div key={item.id} className="flex gap-8 items-center border-b border-nude-50 pb-10 last:border-0">
                <div className="w-28 h-28 rounded-3xl overflow-hidden shrink-0 shadow-md"><img src={item.image} className="w-full h-full object-cover" /></div>
                <div className="flex-1 space-y-2">
                  <h4 className="font-serif text-2xl font-bold text-nude-500">{item.name}</h4>
                  <p className="text-pastel-clay font-bold text-lg">${item.price.toLocaleString()}</p>
                  <div className="flex items-center gap-6 mt-4">
                    <div className="flex items-center gap-4 bg-nude-50 rounded-full px-4 py-2 border border-nude-100">
                      <button onClick={() => onUpdate(item.id, item.quantity - 1)} className="text-xl font-bold text-nude-400 hover:text-nude-500">-</button>
                      <span className="font-bold text-nude-500 min-w-4 text-center">{item.quantity}</span>
                      <button onClick={() => onUpdate(item.id, item.quantity + 1)} className="text-xl font-bold text-nude-400 hover:text-nude-500">+</button>
                    </div>
                    <button onClick={() => onUpdate(item.id, 0)} className="text-xs uppercase tracking-widest font-bold text-red-300 hover:text-red-500 transition-colors">Remove</button>
                  </div>
                </div>
              </div>
            ))}
