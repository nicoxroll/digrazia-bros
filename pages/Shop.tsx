...
export const Shop: React.FC<{ onAddToCart: (p: Product) => void }> = ({
  onAddToCart,
}) => {
  ...
  return (
    <div className="pb-24">
      <section className="relative h-screen flex items-center px-8 overflow-hidden mb-24">
        ...
        >
          <h2 className="font-serif text-5xl sm:text-7xl md:text-[8rem] text-white mb-6 drop-shadow-xl font-bold tracking-tighter">
            Collections
          </h2>
          <p className="text-xl md:text-2xl text-white/90 mb-8 max-w-2xl mx-auto leading-relaxed drop-shadow-lg font-medium">
            Discover our curated selection of artisanal furniture. Each piece
            tells a story of craftsmanship and timeless design.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <button
              onClick={() => window.lenis?.scrollTo(galleryRef.current)}
              className="px-8 py-4 bg-white text-nude-500 rounded-full font-bold uppercase tracking-widest hover:bg-pastel-clay hover:text-white transition-all shadow-xl hover:scale-105"
            >
              Explore Gallery
            </button>
          </div>
      ...
      <div ref={galleryRef} className="max-w-[1920px] mx-auto px-8">
        <div className="mb-8 border-b border-nude-50 pb-6">
          <div className="flex flex-col md:flex-row justify-between items-center gap-8 mb-8">
            <h3 className="font-serif text-5xl text-nude-500 font-bold tracking-tight">
              The Gallery
            </h3>
            <button
              ...
              {categories.map((c) => (
                <button
                  ...
                  className={`px-12 py-4 rounded-full text-xs font-bold uppercase tracking-widest transition-all ${
                    filter === c
                      ? "bg-nude-500 text-white shadow-xl scale-105"
                      : "bg-white text-nude-400 border border-nude-100 hover:bg-nude-50"
                  }`}
                >
                  {c}
                </button>
