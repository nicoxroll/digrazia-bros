...
const ExpandingImageRow: React.FC<{
  images: string[];
...
export const ProductDetail: React.FC<{ onAddToCart: (p: Product) => void }> = ({
  ...
  const handleVisualize = async () => {
    ...
    try {
      ...
      const result = await GeminiService.visualizeInSpace(
        roomImage,
        product,
        visualPrompt,
        furnitureB64
  ...
  return (
    <div className="bg-white">
      ...
      <section className="relative h-screen w-full overflow-hidden flex items-center justify-center bg-nude-50">
        ...
        <button
          onClick={() => navigate(-1)}
          className="absolute top-24 left-8 lg:left-24 z-20 inline-flex items-center gap-2 text-white/80 hover:text-white transition-colors drop-shadow-md"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="18"
        ...
        <div className="absolute bottom-16 left-8 lg:left-24 z-10 max-w-xl">
          <p className="text-white/70 uppercase tracking-[0.3em] font-bold text-xs mb-3 drop-shadow-sm">
            {product.category}
          </p>
          <h1 className="font-serif text-6xl lg:text-8xl text-white mb-6 font-bold tracking-tighter drop-shadow-2xl">
            {product.name}
          </h1>
          <div className="flex items-center gap-8">
            <p className="text-3xl text-white font-serif italic drop-shadow-md">
              ${product.price.toLocaleString()}
            </p>
            <button
              onClick={() => onAddToCart(product)}
              className="bg-white text-nude-500 px-10 py-5 rounded-full font-bold uppercase tracking-widest shadow-2xl hover:bg-pastel-clay hover:text-white transition-all"
            >
              Add to Basket
            </button>
          </div>
      ...
      <section className="max-w-[1920px] mx-auto px-8 py-32 space-y-32">
        <div className="space-y-12 max-w-4xl">
          <span className="text-pastel-clay uppercase tracking-[0.5em] font-bold text-xs">
            The Concept
          </span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
              <div className="space-y-8 text-left">
                <div
                  ...
                  className="aspect-video bg-white rounded-3xl border-2 border-dashed border-nude-200 flex flex-col items-center justify-center cursor-pointer hover:border-pastel-clay transition-all overflow-hidden group"
                >
                  {roomImage ? (
                    <img
                      src={roomImage}
                    ...
                    <p className="text-sm font-bold text-nude-300">
                      Upload Room Photo
                    </p>
                  )}
                  <input
                ...
                {error && (
                  <div className="bg-white border-l-4 border-red-400 p-6 shadow-xl rounded-r-2xl animate-in fade-in slide-in-from-bottom-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-serif text-red-900 font-bold text-lg">
                          Simulation Issue
                        </p>
                ...
                <button
                  ...
                  disabled={!roomImage || isVisualizing}
                  className="w-full bg-nude-500 text-white py-6 rounded-full font-bold uppercase tracking-widest"
                >
              ...
              <div className="aspect-square bg-white rounded-[4rem] shadow-2xl overflow-hidden relative border-8 border-white group">
                {visualResult ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center italic text-nude-200">
                    Your render appears here
                  </div>
                )}
                {isVisualizing && (
                  <div className="absolute inset-0 bg-white/70 backdrop-blur-md flex items-center justify-center animate-pulse" />
                )}
              </div>
            </div>
          </div>
